...
@RestController
@RequestMapping(value = "/rest/permits")
...
public class PermitRestController {
    ...
    public ResponseEntity<?> findAllXml() throws TransformerException, IOException {
        ...
        ClassPathResource xslt = new ClassPathResource("static/permit.xslt");
        Transformer transformer = transformerFactory.newTransformer(new StreamSource(xslt.getInputStream()));
        Source xmlSource = new StreamSource(new ByteArrayInputStream(xmlMapper.writeValueAsBytes(permitService.findAll())));
    ...
    public ResponseEntity<?> findAllJson() {
        return ResponseEntity.status(HttpStatus.OK).body(permitService.findAll());
    }
    ...
    public ResponseEntity<?> save(@RequestBody PermitDto permitDto) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable("id") Long id, @RequestBody PermitDto permitDto) {
        permitDto.setId(id);
        permitService.save(permitDto);
        return new ResponseEntity<>(HttpStatus.OK);
    }
